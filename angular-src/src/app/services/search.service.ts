...
export class SearchService {
  ...
  getReservations(username) {
    ...
    this.searchUrl = 'http://localhost:8080/viewUserReservation';
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post(this.searchUrl, userRes, {headers:headers}).map(res => res.json());
  }
  getClientReservation(client_name) {
    const client_name = {
      client_name: str
    }
    this.searchUrl = 'http://localhost:8080/viewClientReservation';
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post(this.searchUrl, client_name, {headers:headers}).map(res => res.json());
  }
  searchEvents(str) {
    const searchKey = {
      searchKey: str
